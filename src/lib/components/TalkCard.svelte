<script lang="ts">
	...
	const href = $derived(`/talks/${talk.slug}`);
	const isDraft = $derived(talk.status === 'draft');
</script>
...
>
	<div class="flex items-center justify-between gap-3">
		<Tag variant="accent">{talk.year}</Tag>
		{#if isDraft}
			<span
	...
	<p class="font-mono text-fg-muted mt-2 text-xs tracking-[0.1em] uppercase">{talk.event}</p>
	{#if talk.location}
		<p class="text-fg-soft mt-2 text-sm">{talk.location}</p>
	{/if}
</a>
